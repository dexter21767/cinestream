...
function filter(list = Array, meta = Object) {
    ...
    if(SortedResults.sub) SortedResults.sub = SortedResults.sub.filter(value=> value.sortingRank>0);
...
function getSubs(subs = Array,proxy){
    ...
    let subtitles = [];
    subs.forEach(({url,lang},index) => {
        if(url.match(/thumbnails|sprite|images/gi)) return
        sub2vtt.gerenateUrl(url,proxy)
        //let sub = {"url":url};
        let sub = {}
        sub.url = config.local+"/sub.vtt?"+sub2vtt.gerenateUrl(url,proxy);
        iso639.forEach((iso,index)=>{
            if(iso.name.toLowerCase() == lang.toLowerCase()) sub.lang = iso.iso6393
            else sub.lang = lang;     
        })
        sub.id = sub.lang + index
        subtitles.push(sub)
