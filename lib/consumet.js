...
async function scrapper(id,type = String, key = String, query = String, meta = Object, ep, season) {
    try {
        ...
        if(key !== "Fmovies") return
        //const StreamCache = ProviderCache[key];
        if(key == "Fmovies") provider = new MOVIES.Fmovies("https://9anime.enimax.xyz", {url: "https://proxy.vnxservers.com/"}, "848624aaffec43808c86f5e47e3fa5b0");
        ...
        else episode = (key == "FlixHQ" || key == "Fmovies") ? await provider.fetchEpisodeSources(info.episodes[0].id, info.id.slice(1)) : await provider.fetchEpisodeSources(info.episodes[0].id);
...
/*
(async () => {
     const movie = new MOVIES.Fmovies("https://9anime.enimax.xyz", {url: "https://proxy.vnxservers.com/"}, "848624aaffec43808c86f5e47e3fa5b0");
     const search = await movie.search('game');
