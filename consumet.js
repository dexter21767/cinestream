...
async function scrapper(type = String, key = String, query = String, ep,season) {
    try {
        ...
        if(!Cached) episode = key == "FlixHQ"?await provider.fetchEpisodeSources(info[0].id,info.showId):await provider.fetchEpisodeSources(info[0].id);
...
async function getAnime(id,episode) {
    try {
        ...
        for (const key in ANIME) {
            promises.push(animeScrapper(key, query, episode));
        }
        const results = await Promise.allSettled(promises)
        let streams = [];
        let subs = [];
...
async function animeScrapper(key = String, query = String, ep) {
    try {
        //console.log(key);
        let Cached,provider,resultInfo,results,episode,id;
